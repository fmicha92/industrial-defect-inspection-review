...
description: Orchestrate a two-pass literature review for a research question. Phase 1 casts a wide net via scholar-search (and optionally arxiv-search), phase 2 the agent screens candidates by title+abstract, phase 3 the shortlist gets full-text analysis, phase 4 the agent writes a structured synthesis. Persistent session state lives in the project at literature-reviews/<slug>/. Use when the user says "literature review", "survey the field", "research question", "what does the literature say about", or "systematic search".
license: MIT
...
2. search    Wide net across OpenAlex via scholar-search (+ arXiv optional), dedup
3. screen    Agent reads titles/abstracts/metadata, marks keepers
...
# Returns: {"session": "/path/to/project/literature-reviews/what-makes-sparse-..", "slug": "..."}

# 2. Cast a wide net
python3 scripts/literature_review.py search <session-dir> \
    --limit 50 --year 2020- --min-citations 25 \
    [--venue ICLR] [--include-arxiv]
...
# 5-6. Agent reads each paper and writes <project-root>/literature-reviews/<slug>/final.md
...
### Phase 2: search
Default limit 50, year floor 2020, and citation floor 25. Use `--year` or
`--min-citations` only when the user asks for a different scope. Add
`--include-arxiv` only when the user explicitly wants preprints or
lower-citation coverage, because arXiv results may not have citation counts.

Script dedups by arxiv_id, doi, and lowercased title.

If the review is specifically a search for **new papers to add to the vault**,
or the user asks for suggestions not already represented in the vault, run the
`paper-semantic-relations` search workflow or otherwise screen `candidates.json`
against existing `obsidian/Papers/` notes before presenting suggestions.
Duplicate exclusion must check DOI, arXiv ID, OpenAlex ID, normalized title,
paper-note filename, aliases, and fuzzy title match.
...
When showing candidate or shortlist suggestions to the user, include a Markdown
hyperlink to OpenAlex when possible. Prefer `openalex_url`; if it is missing but
`paper_id` is an OpenAlex work ID, link to `https://openalex.org/<paper_id>`. If
no OpenAlex ID is available, write `not available`.

Unless the user explicitly asks for older or lower-citation papers, candidate
and shortlist suggestions must be from 2020 or newer and have at least 25
citations.

For new-paper suggestion lists, exclude candidates already present in the vault
before showing the table. If an already-known paper is contextually important,
mention it separately as "already in vault" rather than counting it as a
suggestion.
...
- **Never fabricate citations, abstracts, or metadata.** If a field is missing in `shortlist.json`, mark it as such in the synthesis.
...
- **Write standalone review prose.** Do not use vault-relative or workflow
  metacommentary in `final.md`; keep session, shortlist, fetch-plan, and script
  details out of the synthesis except for bounded full-text status notes in the
  appendix.
