...
Sessions live in <project-root>/literature-reviews/<slug>/ by default
(overridable). Each
...
Usage:
    ...
    literature_review.py search  <session-dir> [--limit 50] [--year 2020-] \\
                                  [--venue ICLR] [--min-citations 25]
...
DEFAULT_OUT_DIR = PROJECT_ROOT / "literature-reviews"
...
def cmd_screen(args) -> int:
    ...
    # Accept OpenAlex paper_id, arxiv_id, or DOI as the selector.
...
def main() -> int:
    ...
    p_search.add_argument("--year", default="2020-",
                          help="Year range, e.g. 2022-2026, 2024, or 2020- (default 2020-)")
    p_search.add_argument("--venue")
    p_search.add_argument("--min-citations", type=int, default=25,
                          help="Minimum cited-by count for scholar results (default 25)")
    ...
    p_screen.add_argument("--include", required=True,
                          help="Comma-separated IDs (OpenAlex paper_id, arxiv_id, or DOI) to keep")
