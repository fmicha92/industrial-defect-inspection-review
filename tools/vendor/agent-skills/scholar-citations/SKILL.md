...
- `--min-year YYYY` — drop older entries (default 2020)
- `--min-citations N` — drop low-impact entries (default 25)
...
When presenting referenced or citing papers as suggestions, include a Markdown
...
must be from 2020 or newer and have at least 25 citations. For historical
ancestry, override with a lower `--min-year` or `--min-citations`.

## Rate limits

OpenAlex is generous: 10 req/sec anonymous, more in the polite pool (set `OPENALEX_EMAIL`). Script self-throttles at 5 req/sec using a shared cooldown file in the project at `.cache/ai-skill-scholar/` by default. Override with `AI_SKILL_SCHOLAR_CACHE=/path/to/cache` if needed.
