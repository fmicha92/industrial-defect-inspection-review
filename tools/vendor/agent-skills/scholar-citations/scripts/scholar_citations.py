...
Usage:
    python scholar_citations.py references <id> [--limit 100] [--min-year 2020] [--min-citations 25]
    python scholar_citations.py citations  <id> [--limit 100] [--min-year 2020] [--min-citations 25]
...
CACHE_ROOT = Path(
    os.environ.get(
        "AI_SKILL_SCHOLAR_CACHE",
        str(PROJECT_ROOT / ".cache" / "ai-skill-scholar"),
    )
)
...
def main() -> int:
    ...
    parser.add_argument("--min-citations", type=int, default=25,
                        help="Minimum cited-by count (default 25)")
