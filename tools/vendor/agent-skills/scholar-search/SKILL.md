...
```bash
# Basic query (defaults to 2020+ and 25+ citations)
...
- `--year YYYY`, `YYYY-YYYY`, or `YYYY-` — publication year filter (default `2020-`)
...
- `--min-citations N` — quality floor (default 25)
...
### 1. Parse intent
- "find papers on X" → basic query with default `--year 2020- --min-citations 25`
...
If the user asks for **new papers**, "papers not already in the vault",
recommendations to add, or literature expansion grounded in existing vault
notes, use `paper-semantic-relations` instead of presenting raw scholar-search
results. Raw OpenAlex search does not know which papers are already represented
in `obsidian/Papers/`.
...
### 3. Present results
Compact table: citation count | year | venue | authors | title | OpenAlex | first-sentence-of-abstract. Sort by citation count unless the user asked for recency.

For every paper suggestion, include a Markdown hyperlink to OpenAlex when
possible. Prefer `openalex_url`; if it is missing but `paper_id` is an OpenAlex
work ID, link to `https://openalex.org/<paper_id>`. If no OpenAlex ID is
available, write `not available`.

Unless the user explicitly asks for older or lower-citation papers, suggestions
must be from 2020 or newer and have at least 25 citations. Do not present
lower-citation or pre-2020 papers as suggestions by default.

For vault-expansion suggestions, do not present this table until each result has
been screened against existing paper notes by DOI, arXiv ID, OpenAlex ID,
normalized title, paper-note filename, aliases, and fuzzy title match. Prefer
the `paper-semantic-relations` helper because it performs this exclusion and
returns `excluded_existing`.
...
Script self-throttles at 5 req/sec per process. The shared cooldown file is stored in the project at `.cache/ai-skill-scholar/` by default. Override with `AI_SKILL_SCHOLAR_CACHE=/path/to/cache` if needed.
